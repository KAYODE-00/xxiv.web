...
export const useVersionsStore = create<VersionsStore>((set, get) => ({
  ...
  loadVersionHistory: async (entityType, entityId, currentStateHash?: string) => {
    ...
    try {
      const response = await fetch(
        `/xxiv/api/versions?entityType=${entityType}&entityId=${entityId}&limit=100`
      );
  ...
  undo: async (entityType, entityId) => {
    ...
    if (!version) {
      try {
        const response = await fetch(`/xxiv/api/versions/${versionId}`);
        const result = await response.json();
        if (result.data) {
          version = result.data;
          set((state) => ({
    ...
    if (newUndoStack.length > 0) {
      ...
      if (!previousVersion) {
        try {
          const response = await fetch(`/xxiv/api/versions/${previousVersionId}`);
          const result = await response.json();
          if (result.data) {
