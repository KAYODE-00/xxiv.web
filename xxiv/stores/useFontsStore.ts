...
export const useFontsStore = create<FontsStore>((set, get) => ({
  ...
  loadFonts: async () => {
    ...
    try {
      const response = await fetch('/xxiv/api/fonts');
  ...
  deleteFont: async (fontId: string) => {
    try {
      const response = await fetch(`/xxiv/api/fonts/${fontId}`, {
        method: 'DELETE',
      });
  ...
  loadGoogleFontsCatalog: async () => {
    ...
    try {
      const response = await fetch('/xxiv/api/fonts/google');
  ...
  injectFontsCss: (iframeDocument?: Document | null) => {
    ...
    const styleId = 'xxiv-fonts-style';
