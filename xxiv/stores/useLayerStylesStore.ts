...
export const useLayerStylesStore = create<LayerStylesStore>((set, get) => ({
  ...
  loadStyles: async () => {
    ...
    try {
      const response = await fetch('/xxiv/api/layer-styles');
  ...
  updateStyle: async (id, updates) => {
    ...
    try {
      const response = await fetch(`/xxiv/api/layer-styles/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  restoreLayerStyles: async (styleIds) => {
    ...
    for (const styleId of stylesToRestore) {
      try {
        const response = await fetch(`/xxiv/api/layer-styles/${styleId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
