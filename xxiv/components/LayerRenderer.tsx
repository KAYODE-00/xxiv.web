...
import LayerContextMenu from '@/app/(builder)/xxiv/components/LayerContextMenu';
import CanvasTextEditor from '@/app/(builder)/xxiv/components/CanvasTextEditor';
...
const LayerItem: React.FC<{
  ...
  const originalImageAlt = rawImageAlt.includes('<xxiv-inline-variable>')
  ...
  const renderContent = () => {
    ...
    if (htmlTag === 'form' && !isEditMode) {
      ...
      elementProps.onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
          const response = await fetch('/xxiv/api/form-submissions', {
            method: 'POST',
    ...
    if (layer.name === 'localeSelector' && !isEditMode && availableLocales && availableLocales.length > 0) {
      ...
      const currentPageSlug = typeof window !== 'undefined'
        ? window.location.pathname.slice(1).replace(/^xxiv\/preview\/?/, '')
        : '';
