...
export default function PasswordForm({ pageId, folderId, redirectUrl, isPublished = true }: PasswordFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="xxiv-password-form">
      <div className="xxiv-password-form-field">
        <input
          type="password"
          value={password}
          onChange={(e) => setPassword(e.target.value)}
          placeholder="Enter password"
          className="xxiv-password-input"
          disabled={isLoading}
          autoFocus
          required
        />
      ...
      {error && (
        <div className={`xxiv-password-error ${isRateLimited ? 'xxiv-password-rate-limited' : ''}`}>
          {isRateLimited && (
            <svg
              width="16" height="16"
      ...
      <button
        ...
        className="xxiv-password-submit"
      >
      ...
      <style jsx>{`
        ...
        .xxiv-password-input:focus {
          border-color: #3b82f6;
          box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1);
        }

        .xxiv-password-input:disabled {
        ...
        .xxiv-password-submit:hover:not(:disabled) {
          background-color: #2563eb;
        }

        .xxiv-password-submit:disabled {
