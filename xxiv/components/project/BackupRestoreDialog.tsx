...
export function BackupRestoreDialog({
  ...
  const handleBackup = async () => {
    ...
    try {
      const response = await fetch('/xxiv/api/project/export', {
        method: 'POST',
      ...
      const filename = filenameMatch?.[1] || 'backup.xxiv';
  ...
  const handleRestore = async () => {
    if (!selectedFile) {
      toast.error('No backup file selected', { description: 'Please select a .xxiv backup file' });
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/xxiv/api/project/import', {
        method: 'POST',
        body: formData,
      });
      ...
      handleClose();
      setTimeout(() => { window.location.href = '/xxiv'; }, 500);
    } catch (err) {
      ...
      >
        ...
        >
          ...
          <TabsContent value="backup">
            <div className="flex flex-col gap-4 pt-2">
              <p className="text-xs text-muted-foreground">
                This will create a <code className="text-foreground/85">.xxiv</code> backup file containing all of your project data. The file can be used to restore
                your project data at a later date or to transfer your project to another instance of XXIV.
              </p>
          ...
          <TabsContent value="restore">
            <div className="flex flex-col gap-4 pt-2">
              <p className="text-xs text-muted-foreground">
                Upload a <code className="text-foreground/85">.xxiv</code> backup file to restore a project. Warning: This will delete all the current project data
