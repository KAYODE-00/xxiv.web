...
import {
  componentsUpdatesChannelForSegment,
  pagesUpdatesChannelForPage,
} from '@/lib/xxiv/realtime-namespace';
import type { Component, Layer, Page } from '@/types';

const MCP_USER_ID = '__mcp_agent__';
const MCP_COMPONENTS_CHANNEL = componentsUpdatesChannelForSegment('legacy-shared');
...
export async function broadcastPageCreated(page: Page): Promise<void> {
  await broadcast(pagesUpdatesChannelForPage(page), 'page_created', page as unknown as Record<string, unknown>);
}
...
export async function broadcastPageUpdated(
  ...
  await broadcast(pagesUpdatesChannelForPage(page), 'page_update', {
    page_id: page.id,
    user_id: MCP_USER_ID,
    changes,
    timestamp: Date.now(),
...
export async function broadcastPageDeleted(page: Page): Promise<void> {
  await broadcast(pagesUpdatesChannelForPage(page), 'page_deleted', {
    pageId: page.id,
    user_id: MCP_USER_ID,
    timestamp: Date.now(),
  });
...
export async function broadcastComponentCreated(component: Component): Promise<void> {
  await broadcast(MCP_COMPONENTS_CHANNEL, 'component_created', {
...
export async function broadcastComponentUpdated(
  ...
  await broadcast(MCP_COMPONENTS_CHANNEL, 'component_updated', {
...
export async function broadcastComponentDeleted(componentId: string): Promise<void> {
  await broadcast(MCP_COMPONENTS_CHANNEL, 'component_deleted', {
    component_id: componentId,
    user_id: MCP_USER_ID,
    timestamp: Date.now(),
  });
...
export async function broadcastComponentLayersUpdated(
  ...
  await broadcast(MCP_COMPONENTS_CHANNEL, 'component_layers_updated', {
    component_id: componentId,
    layers,
    user_id: MCP_USER_ID,
    timestamp: Date.now(),
