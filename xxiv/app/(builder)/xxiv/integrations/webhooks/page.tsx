...
export default function WebhooksPage() {
  ...
  const fetchWebhooks = async () => {
    try {
      const response = await fetch('/xxiv/api/webhooks');
  ...
  const handleSaveWebhook = async () => {
    ...
    try {
      ...
      const url = isEditing
        ? `/xxiv/api/webhooks/${editingWebhook.id}`
        : '/xxiv/api/webhooks';
  ...
  const handleDeleteWebhook = async () => {
    ...
    try {
      const response = await fetch(`/xxiv/api/webhooks/${webhookToDelete.id}`, {
        method: 'DELETE',
      });
  ...
  const handleToggleEnabled = async (webhook: Webhook) => {
    try {
      const response = await fetch(`/xxiv/api/webhooks/${webhook.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleViewDeliveries = async (webhook: Webhook) => {
    ...
    try {
      const response = await fetch(`/xxiv/api/webhooks/${webhook.id}/deliveries?limit=20`);
      const result = await response.json();
    ...
    <div className="p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Receive real-time notifications when events occur in your Xxiv site.
        </p>
