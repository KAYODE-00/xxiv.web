...
export default function AppsPage() {
  ...
  // Auto-open app settings from ?app= query param (e.g. /xxiv/integrations/apps?app=mapbox)
  useEffect(() => {
    if (isLoading) return;
  ...
  const fetchApps = async () => {
    try {
      const response = await fetch('/xxiv/api/apps');
  ...
  const loadMailerLiteSettings = async () => {
    ...
    try {
      const response = await fetch('/xxiv/api/apps/mailerlite/settings');
      const result = await response.json();
  ...
  const handleTestApiKey = async () => {
    ...
    setIsTesting(true);
    try {
      const response = await fetch('/xxiv/api/apps/mailerlite/test', {
        method: 'POST',
  ...
  const handleDisconnect = async () => {
    try {
      await fetch('/xxiv/api/apps/mailerlite/settings', {
        method: 'DELETE',
      });
  ...
  const loadTokenApp = async (appId: string) => {
    ...
    try {
      const response = await fetch(`/xxiv/api/apps/${appId}/settings`);
      const result = await response.json();
  ...
  const handleDisconnectTokenApp = async (appId: string) => {
    ...
    try {
      await fetch(`/xxiv/api/apps/${appId}/settings`, { method: 'DELETE' });
      updateTokenAppState(appId, {
        token: '',
        savedToken: '',
        isConnected: false,
  ...
  const loadGroupsAndForms = useCallback(async () => {
    ...
    try {
      const [groupsRes, formsRes] = await Promise.all([
        fetch('/xxiv/api/apps/mailerlite/groups'),
        fetch('/xxiv/api/form-submissions?summary=true'),
      ]);
  ...
  const renderConnectionForm = () => (
    <>
      {/* Form Selection */}
      <Field>
        <FieldLabel>Xxiv Form</FieldLabel>
        {isLoadingForms ? (
