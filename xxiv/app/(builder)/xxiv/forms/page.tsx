...
async function fetchFormSummaries(): Promise<FormSummary[]> {
  const response = await fetch('/xxiv/api/form-submissions?summary=true');
...
async function fetchFormSubmissions(formId: string): Promise<FormSubmission[]> {
  const response = await fetch(`/xxiv/api/form-submissions?form_id=${encodeURIComponent(formId)}`);
  const data = await response.json();
  if (data.error) throw new Error(data.error);
  return data.data || [];
}
...
async function updateSubmissionStatus(id: string, status: FormSubmissionStatus): Promise<void> {
  const response = await fetch(`/xxiv/api/form-submissions/${id}`, {
    method: 'PUT',
