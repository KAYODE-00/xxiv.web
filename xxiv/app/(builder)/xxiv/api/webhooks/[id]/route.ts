...
export async function POST(
  ...
  try {
    ...
    const testPayload = {
      ...
      data: {
        message: 'This is a test webhook from Xxiv',
    ...
    if (webhook.secret) {
      const signature = createHmac('sha256', webhook.secret)
        ...
        .digest('hex');
      headers['X-Xxiv-Signature'] = `sha256=${signature}`;
    }
