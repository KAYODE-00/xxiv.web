...
/**
 * POST /xxiv/api/project/import
 *
 * Import a project dump (.xxiv file).
 * Accepts multipart form-data with:
 *   - "file" (required): the .xxiv file
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file || !(file instanceof Blob)) {
      return noCache({ error: 'No file provided. Upload a .xxiv file as form-data with field name "file".' }, 400);
    }
    ...
    try {
      ...
      return noCache({ error: err instanceof Error ? err.message : 'Invalid .xxiv file.' }, 400);
    }
    ...
    console.error('[POST /xxiv/api/project/import] Error:', error);
    return noCache(
