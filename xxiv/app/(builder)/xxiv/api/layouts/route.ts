...
const LAYOUT_ASSETS_DIR = path.join(process.cwd(), 'public', 'xxiv', 'layouts', 'assets');
 ...
 * and save them to /public/xxiv/layouts/assets/.
...
async function downloadAndLocalizeAssets(
  ...
  for (const assetId of uniqueIds) {
    ...
    try {
      ...
      if (existingFilename) {
        localUrlMap[assetId] = `/xxiv/layouts/assets/${existingFilename}`;
        continue;
      }
      ...
      localUrlMap[assetId] = `/xxiv/layouts/assets/${finalFilename}`;
    } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (imageFile) {
      const layoutsDir = path.join(process.cwd(), 'public', 'xxiv', 'layouts', 'previews');
    ...
    const finalImagePath = `/xxiv/layouts/previews/${uniqueLayoutKey}${imageExtension}`;
