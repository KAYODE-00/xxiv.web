...
export default function EmailSettingsPage() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/xxiv/api/settings/email');
        if (response.ok) {
  ...
  const handleTestConnection = async () => {
    try {
      ...
      const response = await fetch('/xxiv/api/settings/email/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Xxiv Servers option — disabled on opensource */}
          <div className="relative flex items-start gap-3 rounded-lg border p-4 opacity-50 cursor-not-allowed bg-secondary/20">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-muted-foreground">Xxiv</span>
                <span className="inline-flex items-center rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground">
                  Only available on Cloud
                </span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                Managed email delivery powered by Xxiv — no configuration required
              </p>
            </div>
          </div>
